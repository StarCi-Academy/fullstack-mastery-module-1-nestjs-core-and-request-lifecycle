...
func ErrorHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Run the next handler (or chain of handlers) and then resume here.
		// Panics are captured by the deferred recover above; non-panic errors
		// are attached via c.Error() by handlers and classified below.
		c.Next()

		// If no errors were attached, the handler completed successfully — skip envelope.
		if len(c.Errors) == 0 {
			return
		}
